...
class GameRacko(QMainWindow, MainWindow):
    ...
    def _default_setting(self):
        ...
        self._racks_labels = [[self.player1Slot01, self.player1Slot02, self.player1Slot03, self.player1Slot04, self.player1Slot05, 
    ...
    def _round_scores_update(self, round_number, scores, isGameEnd):
        ...
        for player in range(self._number_of_players):
            self._scores_labels[self._players_layout[player]].setText(
                str(scores[player]))
        if isGameEnd  and self._game_active:
    ...
    def _refresh_draw_pile(self, count, value, face_up):
        ...
        if face_up:
            ...
            if value > 0 and value <= self._racko_size:
    ...
    def _pick_card(self, id, slot):
        if id == 1 and not self._layout1_lock:
            self._game_engine.humanResponse(slot - 1)
            self._use_discard_card = False
            self._layout1_lock = True
            self._game_engine.start()
        elif id == 3 and not self._layout3_lock:
            self._game_engine.humanResponse(slot - 1)
            self._use_discard_card = False
            self._layout3 = True
            self._game_engine.start()
