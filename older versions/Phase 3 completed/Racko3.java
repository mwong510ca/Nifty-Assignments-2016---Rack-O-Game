...
public class Racko3 extends JPanel implements MouseListener, ActionListener{
    ...
    private void newRound() {
        ...
        if (userDealer) {
            ...
            compHands[0].setHand(initialHands[1]);            
        } else {
            humanHand = initialHands[1];
            compHands[0].setHand(initialHands[0]);
    ...
    public boolean find_and_replace(Card newCard, int numberOfCard, Card[] rack) {
        ...
        compHands[0].discardAdd(rack[index]);
        rack[index] = newCard;
        return true;
    }
    ...
    public void computer_play() {
        ...
        if (compHands[0].determine_use(discard.peek().getValue(), true)) {
            System.out.println("The computer chose to keep the card " + discard.peek().getValue() + ".");
            Card discardCard = discard.pop();
            compHands[0].discardRemove();
            Card returnCard = compHands[0].replace(discardCard, true);
            ...
            if (compHands[0].determine_use(drawCard.getValue(), false)) {
                System.out.println("The computer chose to keep the draw card.");
                Card returnCard = compHands[0].replace(drawCard, false);
                add_card_to_discard(returnCard);    
            } else {
        ...
        if (check_racko(compHands[0].getHand())) {
            b1.setVisible(false);
            b2.setVisible(false);
            playerScore += scoreHand(humanHand);
            computerScore += scoreHandBonus(compHands[0].getHand());
            computerWins++;
            if (playerScore >= 500 || computerScore >= 500) {
                ...
                print_human_hand();
                System.out.printf("computer - %3d : ", scoreHandBonus(compHands[0].getHand()));
                compHands[0].printAll();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // Select button to replace a card in the hand with the selected card
            ...
            if (takeDiscardCard) {
                compHands[0].playerCard(takeCard, 0);
            }
            ...
            selectedSlot = -1;
            takeCard = null;
            takeDiscardCard = false;
            
            if (check_racko(humanHand)) {
                b1.setVisible(false);
                b2.setVisible(false);
                playerScore += scoreHandBonus(humanHand);
                computerScore += scoreHand(compHands[0].getHand());
                playerWins++;
                if (playerScore >= 500 || computerScore >= 500) {
                    if (computerScore > playerScore) {
                        status.setText("Game ended - Computer win!");
                    ...
                    print_human_hand();
                    System.out.printf("computer - %3d : ", scoreHand(compHands[0].getHand()));
                    compHands[0].printAll();
        ...
        // Select button to discard card drawn from deck
            ...
            b5.setVisible(false);
            deckPopped = false;
            Card throwCard = deck.pop();
            System.out.println("The player placed " + throwCard.getValue() + " in discard pile.");
            
            status.setText("Card discarded: " + throwCard.getValue());
            add_card_to_discard(throwCard);
            computer_play();
